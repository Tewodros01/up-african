<script setup>
// Define brands array for Swiper slides
const brands = [
...
<template>
  <section class="mb-8">
    <div
      class="flex flex-col md:flex-row items-center justify-between border-t border-b border-gray-300 py-3 px-4 md:px-10"
    >
      <!-- Title Section -->
      <!-- Title Section -->
      <!-- Title Section -->
      <h2
        class="text-center md:text-left font-medium text-base mb-4 md:mb-0 w-full md:w-[175px]"
      >
        Connecting You with
        <span class="font-semibold text-sm block">Top Industry Leaders</span>
        <span class="font-semibold text-base block">Trust Us</span>
      </h2>
      ...
      <div class="w-full md:w-[calc(100%-200px)]">
        ...
        >
          <SwiperSlide v-for="(brand, i) in brands" :key="i">
            <div class="flex items-center justify-center h-[100px]">
              <img :src="brand" alt="brand" class="w-[65px] object-contain" />
            </div>
          </SwiperSlide>
        </Swiper>
      </div>
